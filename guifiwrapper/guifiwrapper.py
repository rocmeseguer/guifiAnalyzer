...
class CNMLWrapper(object):
    ...
    def __init__(self, rootZoneId=None):
        ...
        self.totallinks = {}
        temp = flatten(
            (self.guifizone.allsubzones).values() + [self.guifizone])
        for zone in temp:
        ...
        if self.totallinks and self.totalnodes:
            logger.info('Working nodes per total nodes: %s ', float(
                len(self.nodes)) / float(len(self.totalnodes)))
            logger.info('Total links per total nodes: %s ', float(
                len(self.totallinks)) / float(len(self.totalnodes)))
            if self.nodes:
                logger.info('Working links per working nodes: %s ', float(
                    len(self.links)) / float(len(self.nodes)))
            logger.info('Working links per total links : %s ', float(
                len(self.links)) / float(len(self.totallinks)))
            nonworklinks = [
                i for i in self.totallinks.values() if i.status != libcnml.Status.WORKING]
            logger.info('Non Working (status) links: %s (%s percent of total links)', len(
                nonworklinks), float(len(nonworklinks)) / float(len(self.totallinks)))
            unparsedlinks = [i for i in self.totallinks.values() if (
                not i.nodeB) and i.status == libcnml.Status.WORKING]
            logger.info('Unparsed Working links: %s (%s percent of total links)', len(
                unparsedlinks), float(len(unparsedlinks)) / float(len(self.totallinks)))
            selflinks = [i for i in self.totallinks.values(
            ) if i.nodeA == i.nodeB and i.status == libcnml.Status.WORKING]
            logger.info('Self links: %s (%s percent of total links)',
                        len(selflinks),
                        float(len(selflinks)) / float(len(self.totallinks)))
            cablelinks = [i for i in self.totallinks.values(
            ) if i.type == "cable" and i.status == libcnml.Status.WORKING]
            logger.info('Cable links: %s (%s percent of total links)',
                        len(cablelinks),
        ...
        # Todo Fix broken links? The ones not recognized cause node in other
        # zone
    def findAttributeTypes(self):
        ...
        attr = int(
            raw_input("Enter: 1 for devices, 2 for ifaces, 3 for links or 4 for Services: "))
        if attr == 1:
...
class GuifiZone(object):
    ...
    def __init__(self, zone):
        ...
        logger.info('Total %s (%s) nodes:  %s', self.zone.id,
                    self.zone.title, len(self.totalnodes))
        logger.info('Total %s (%s) links:  %s', self.zone.id,
                    self.zone.title, len(self.totallinks))
        logger.info('Working %s (%s) nodes:  %s', self.zone.id,
                    self.zone.title, len(self.nodes))
        logger.info('Working %s (%s) links:  %s', self.zone.id,
                    self.zone.title, len(self.links))
        if self.totallinks and self.totalnodes:
            logger.info('Working nodes per total nodes: %s ', float(
                len(self.nodes)) / float(len(self.totalnodes)))
            logger.info('Total links per total node: %s ', float(
                len(self.totallinks)) / float(len(self.totalnodes)))
            if self.nodes:
                logger.info('Working links per working nodes: %s ', float(
                    len(self.links)) / float(len(self.nodes)))
            ...
            logger.info('Non Working (status) links: %s (%s percent of total links)', len(
                nonworklinks), float(len(nonworklinks)) / float(len(self.totallinks)))
            self.unparsedlinks = [i for i in self.totallinks.values() if (
                not i.nodeB) and i.status == libcnml.Status.WORKING]
            logger.info('Unparsed Working links: %s (%s percent of total links)', len(
                self.unparsedlinks), float(len(self.unparsedlinks)) / float(len(self.totallinks)))
            selflinks = [i for i in self.totallinks.values(
            ) if i.nodeA == i.nodeB and i.status == libcnml.Status.WORKING]
            logger.info('Self links: %s (%s percent of total links)',
                        len(selflinks),
        ...
        logger.info('%s subzones of %s (%s): %s', len(
            self.allsubzones), self.zone.id, self.zone.title, self.allsubzones)
    ...
    def getAllSubZonesHelper(guifizone):
        ...
        if subzones == []:
            ...
            # print subzones
            # return subzones.update({x.getAllSubZones() for x in
            # subzones.values()})
    ...
    def getCNMLZoneWorking(self, zoneIn):
        ...
        """
        # TODO fix counters from upper elements (for example node counter in
        # zone) OR MAYBE not necessary

        # Can parse zones as indepent since their data are not crossing one
        # another
        ...
        nodes = {node.id: cnmlObjectCopy(
            node) for node in zone.getNodes() if node.status == libcnml.Status.WORKING}
        zone.nodes = nodes
        ...
        for node in zone.getNodes():
            ...
            node.devices = {device.id: cnmlObjectCopy(
                device) for device in node.getDevices() if device.status == libcnml.Status.WORKING}
            node.services = {service.id: cnmlObjectCopy(
                service) for service in node.getServices() if service.status == libcnml.Status.WORKING}
            ...
            for device in node.getDevices():
                ...
                for interface in device.getInterfaces():
                    interface.links = {link.id: cnmlObjectCopy(
                        link) for link in interface.getLinks() if link.status == libcnml.Status.WORKING}
                    # Remove links that reach outside the zone
                    interface.links = {
                        link.id: link for link in interface.getLinks() if isinstance(
                            link.nodeB,
                            libcnml.libcnml.CNMLNode)}
                    # Remove self-links
                    interface.links = {
                        link.id: link for link in interface.getLinks() if link.nodeB.id != link.nodeA.id}
    ...
    def getZoneElements(self, zone):
        ...
        # return
        # {'devices':devices,'services':services,'radios':radios,'ifaces':ifaces,
        # 'links':links}
    ...
    def getNodeElements(self, node):
        ...
        # return
        # {'devices':deviceIds,'services':serviceIds,'radios':radioIds,'ifaces':ifaceIds,
        # 'links':linkIds}
        return {
            'devices': devices,
            'services': services,
        ...
        for node in zone.getNodes():
            for device in node.getDevices():
                for radio in device.getRadios():
                    for iface in radio.getInterfaces():
                        newLinks = [
                            l for l in iface.getLinks() if l not in links]
    ...
    def createTopoJSON(self):
        ...
        for link in self.nodes.values():
            ...
            # ERROR not working cause of logical error descired down
            parent = getParentCNMLNode(link)
...
# TODO create dictionaries with all the working elements (like dics of
# CNMLParser)
            ...
            for link in u:
                if link.nodeA not in g.nodes:
                    if link.nodeA not in g.totalnodes:
                        logger.warning(
                            'Link: %s NodeA: %s is not in parsed CNML',
                            link.id,
                            link.nodeA)
                        continue
                    else:
                        logger.warning(
                            'Link: %s  NodeA: %s is in not in WORKING mode !Weir since link status is %s',
                            link.id,
                            link.nodeA,
                            link.status)
                        continue
                ...
                if getParentCNMLNode(link).id not in g.nodes:
                    logger.warning(
                        'Link: %s  ParentNode: %s is not WORKING mode !Weird since link status is %s',
                        link.id,
                        link.nodeA,
                        link.status)
                    continue
                ...
                #    continue
                logger.info(
                    'Unparsed link %s Status: %s Type: %s Link.nodeA: %s NodeAZone: %s Link parent Node: %s Parent Node Zone %s',
                    link.id,
                    link.status,
                    link.type,
                    link.nodeA,
                    (g.nodes[
                        link.nodeA]).parentZone.id,
                    getParentCNMLNode(link).id,
                    (g.nodes[
                        getParentCNMLNode(link).id]).parentZone.id)
